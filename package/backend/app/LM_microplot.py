...
def load(config_path):
    ...
    """
    m = read_from_config(config_path)
    ...
    def _plotter_play_clb(play_object_str: bytes):
        ...
        micro_task(
            tag=m.file_session_task_tag, task=__file_reader(sketch_name, workspaces)
        )
        return "text/plain", "ok\n"

    async def __file_reader(sketch_name, workspaces=None, ms_period=50):
        """
    ...
    def _plotter_test_clb(*_):
        ...
        micro_task(
            tag=m.file_session_task_tag, task=__file_reader("test_routine.gcode")
        )
        return "text/plain", "ok\n"
    ...
    micro_task(tag=m.control_task_tag, task=__control_task(m))
